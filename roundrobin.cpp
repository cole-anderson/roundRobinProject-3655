...
int main(void)
{
    ...
    */
    ...
    /*
    loop:
      x(clock time) y(process in execution) z(arriving process) s'(next process
    in ready queue) at Time x: We start process y execution and increment a
    clock +1 check for a) arrival z b) process in ready queue if arrival &&
    ready queue :: add arrival z to end of ready Q THEN add y to end of ready Q
    (behind z) -> start execution of s' at start of ready queue if arrival &&
    !ready queue :: add y to end of ready queue (y = s') -> start s' at start of
    ready queue if !arrival && ready queue :: add y to end of ready queue (y !=
    s') -> start s' at start of ready queue if !arrival && !ready queue ::
    continue y (return top of loop)
    ...
    */
    ...
    while (arr != pNum && complete != pNum) // while there are processes in execution
    {
        ...
        if (arr != pNum) // if processes are still arriving (1)
        {
            ...
            for (r = 0; r <= pNum; r++)
            {
                ...
                if (process[r].arrivalT == t) // checks if current time a process arrives
                {
                    ...
                    // for(int i = 0; i < pNum; i++)
                    // {
                    //   std::cout << "Process: " <<process[i].id << " Arrival
                    //   Time: " << process[i].arrivalT << " Required Time: " <<
                    //   process[i].requiredT
                    //   << " Execution Time: " << process[i].exeT << std::endl;
                    // }
                    //

                    if (readyQueue.empty() && cpu.empty()) // WORKS!! //1
                    {
                        // std::cout << "//IF1(empty q and empty cpu)" <<
                        // std::endl;//Debug
                    ...
                    else if (readyQueue.empty() && !cpu.empty()) // 2
                    {
                        ...
                        // std::cout << "//IF2(empty q and !empty cpu)" <<
                        // std::endl;//debug
                        ...
                        // Check if Process in cpu is done executing if yes
                        // update token if no put on ready Q
                    ...
                    else if (!readyQueue.empty() && !cpu.empty()) // 3
                    {
                        ...
                        // std::endl;//debug std::cout << "///VECTOR DEBUG:
                        // FRONT OF QUEUE BEFORE ARRIVAL PUSHBACK " <<
                        // readyQueue.front().id << std::endl;//debug
                ...
                else if (r == pNum)
                {
                    ...
                    if (readyQueue.empty() && !cpu.empty())
                    {
                        ...
                        {
                            std::cout << "Process" << cpu.front().id << std::endl;
                            std::cout << "^CONTINUES EXECUTION(empty q no new "
                                         "processes)"
                                      << std::endl;
                            std::cout << std::endl;
        ...
        while (arr == pNum && complete != pNum)
        {
            ...
            // quick note this does nothing but seg faulted when i commented it
            // out so!
            for (int i = 0; i < pNum; i++)
            {
                ...
                // " << process[i].exeT <<  "c:" << process[i].completed<<
                // std::endl;
            }
    ...
    // for(int i = 0; i < pNum; i++)
    // {
    //   std::cout << "Process: " <<process[i].id << " Arrival Time: " <<
    //   process[i].arrivalT << " Required Time: " << process[i].requiredT << "
    //   Execution Time: " << process[i].exeT << std::endl;
    //
    // }
