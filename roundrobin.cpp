...
int main(void)
{
  ...
  */
  ...
  for (int i = 0; i < pNum; i++)
  {
    ...
    std::cout << "Process" << process[i].id
              << "|Arrival: " << process[i].arrivalT
              << "|Required: " << process[i].requiredT << std::endl;
  }
  std::cout << std::endl;
  std::cout << std::endl;
  std::cout << std::endl;
  ...
  /*
  loop:
    x(clock time) y(process in execution) z(arriving process) s'(next process in
  ready queue) at Time x: We start process y execution and increment a clock +1
      check for a) arrival z b) process in ready queue
      if arrival && ready queue :: add arrival z to end of ready Q THEN add y to
  end of ready Q (behind z) -> start execution of s' at start of ready queue if
  arrival && !ready queue :: add y to end of ready queue (y = s') -> start s' at
  start of ready queue if !arrival && ready queue :: add y to end of ready queue
  (y != s') -> start s' at start of ready queue if !arrival && !ready queue ::
      ...
      for (r = 0; r <= pNum; r++)
      {
            ...
            t) // checks if current time a process arrives
        {
          ...
          else if (!readyQueue.empty() && !cpu.empty()) // 3
          {
            ...
            // BEFORE ARRIVAL PUSHBACK " << readyQueue.front().id <<
            // std::endl;//debug
